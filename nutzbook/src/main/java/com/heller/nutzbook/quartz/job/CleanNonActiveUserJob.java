package com.heller.nutzbook.quartz.job;
...
public class CleanNonActiveUserJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        Date deadtime = new Date(System.currentTimeMillis() - 24*60*60*1000L); // 一天, 测试的时候可以改成2分钟之类的
        Cnd cnd = Cnd.where("userId", ">", 10)
                .and("createTime", "<", deadtime)
                .and(Cnd.exps("emailChecked", "=", false).or("email", "IS", null));
        int deleted = dao.clear(UserProfile.class, cnd);
        log.debugf("delete %d UserProfile", deleted);

        Sql sql = Sqls.create("delete from $user_table " +
                "where id > 10 " +
                "and not exists (select 1 from $user_profile_table where $user_table.id = user_id ) " +
